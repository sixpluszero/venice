...
public class HeartbeatBasedSystemStoreHealthChecker implements SystemStoreHealthChecker {
  ...
  public Map<String, HealthCheckResult> checkHealth(String clusterName, Set<String> systemStoreNames) {
    ...
    // Phase 2: Poll for heartbeat reads. Each polling iteration marks a store HEALTHY (fresh heartbeat) or
    // UNHEALTHY (stale/unreachable). Stores that are never polled (because the periodic check aborts before
    // reaching them) are intentionally left out of the results map, per the SystemStoreHealthChecker contract
    // — the caller treats missing entries as "deferred" rather than UNHEALTHY.
